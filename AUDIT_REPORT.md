...
- **Status:** `RESOLVED` 2026-08-26 by **ADR-033** — recommendation (a) adopted: Re-run always re-executes the persisted statement deterministically, never re-invokes the model; "✎ Refine with AI" is named as the distinct, separately-gated action and was, at that point, explicitly NOT built. Implemented in `services/custom-reports.ts`'s `runRawSqlMode`, proven by `test/custom-reports.test.ts` (asserts the re-run path never imports the AI chat loop). docs/06 §1 and docs/10 §2 updated. **"✎ Refine with AI" itself shipped 2026-08-26** (same day, later slice) — `services/ai-chat.ts`'s `runAskAi` gained an optional seed context, and `applyRefinement` is the one path allowed to materialize a predefined clone into literal SQL, distinct from the hand-edit SQL tab which still refuses to (docs/06 §1). · **Severity:** Medium
...
**C21 — The fee tables carry no index the analytics filters can use** *(new 2026-08-27)*
- **Status:** `OPEN` — measured, and awaiting an owner on the ERP side · **Severity:** High (latency budget, docs/09; affects every fee dashboard at 1,500-school scale)
- **Document:** docs/09 §3 latency budgets vs the `ai_analysis` extract; the cost note in `apps/mcp-server/src/reports/catalog.ts`.
- **Issue.** `fee_compile_data_set` (1.52M rows) has no index but its primary key. `fee_collection_data_set` (1.78M rows) has one secondary index, `idx_fee_collection_delete (society_db, school_db, feedepositid)` — and it is unusable here, because a composite index is only usable from its leftmost column inward and every analytics query filters `school_db`, never `society_db`. Every predefined fee query is therefore a full scan. Measured on the local extract against `stmarksmb`: twelve statements, **all twelve `type=ALL key=NULL`**, 15,648 ms in total, ~1.45–1.70M rows examined each.
- **The fix, and what it is worth.** `ADD INDEX idx_school_year (school_db, academicyearname)` on both tables. Rebuilt on a local copy, the same twelve statements go to **2,423 ms — every one `type=ref key=idx_school_year`** — and `EXPLAIN ANALYZE` puts the actual rows examined at **70,233 against 1,454,684**, a 20.7× reduction. End to end through the orchestrator with a cold Redis tier, Fee Collection lands at **758 ms** and Fee Defaulters at **609 ms**, inside the docs/09 budget with headroom. Reproduce with `npm run bench:fee-index -w @sap/mcp-server`.
- **The column order is load-bearing.** `school_db` leads. Both predicates are equalities, so either order serves the dashboards — but `get_dimensions` filters on the scope column alone, with no year, and a `(academicyearname, school_db)` index would leave those queries on a full scan for nothing.
- **What it costs, so the schema owner can weigh it.** +52 MB on `fee_compile_data_set`, +63 MB on `fee_collection_data_set` — about 16% on top of 733 MB of data. Both ALTERs ran `ALGORITHM=INPLACE, LOCK=NONE` in 13.1 s and 15.5 s, so **no downtime window is required**. The clause was stated explicitly rather than left to the server default, so a fallback to a locking table copy would have been an error rather than a surprise. **Not yet measured: the write cost.** Every INSERT now maintains two more B-tree entries. On a read replica this should be immaterial, but it is unquantified here and it is the first question the schema owner is likely to ask.
- **Why this platform cannot simply fix it.** `ADD INDEX` is a schema write to a school database. Invariant 3 and ADR-008 make the entire data plane read-only, and the `analytics_ro` user holds `GRANT SELECT ON ai_analysis.*` and nothing else — verified, not assumed. The index measured above exists **only on the local development copy**; all 1,500 production databases are untouched and still scanning. This finding is the request, not the change.
- **Classification:** MISSING (schema input owed by the ERP team).
- **Recommended action:** send the two `ALTER` statements and the numbers above to the ERP schema owner, with the no-downtime evidence attached. **This finding needs a named owner on the ERP side — it does not have one yet**, which is the specific reason it is recorded here instead of remaining a code comment nobody was tracking. Related: a covering index for the `get_dimensions` queries (which improved only 2–3×, since they still fetch each row to read `feecategory`/`componentname`) is deliberately **not** proposed — those sit behind the dimension cache (docs/03 §5) and more indexes mean more write cost for a path that is already fast enough.
...
28. **Fee-table indexes — who owns the schema, and will they take the change?** Two `ADD INDEX (school_db, academicyearname)` statements, measured at 6.5× on the query set and no downtime to apply. Unlike the other items here this one is fully specified and evidenced; what is missing is a name on the ERP side to send it to. *(C21 — new 2026-08-27)*
