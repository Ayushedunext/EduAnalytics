...
export async function buildHomeSummary(args: {
   ...
   *
   * -- Why a table and not a strip of cards ------------------------------------
   * The same reason Comparative Analysis' identical panel gives: the shared
   * renderer gathers every `kpi` widget in a spec into the one strip at the top
   * (chart-spec react/ChartSpecView.tsx), which is what makes every screen in
   * this product lay out the same way — so a second strip lower down would be a
   * renderer change, not a report's decision. Three columns say what the cards
   * would: what the highlight is, which school it names, and the number.
   *
   * -- Why it costs no query ---------------------------------------------------
   * Every row is a regrouping of the fee and attendance rows the KPI strip
   * already read (`rollupBySchool`). Home is the screen every user loads, so a
   * panel that needed its own fan-out would be paid for by every launch in the
   * platform.
   *
   * -- Two or more schools, or nothing ----------------------------------------
   * Every row here is a COMPARISON — lowest, largest, average. With one school
   * in scope "largest outstanding" names the only school there is, which is not
   * a finding, and "average across 1 school" is the school. A Principal gets no
   * panel rather than a panel of tautologies; the figures themselves are all in
   * the strip above and in the reports below.
   *
   * -- Ordered by what needs attention, not by what is good -------------------
   * Comparative Analysis' panel opens with "Best recovery". This one opens with
   * the problem, deliberately: it sits on the landing page, where the first row
   * is the most-read line on the screen, and spending it on the school that is
   * doing well is spending it on the school nobody needs to open.
   */
  const rollups = rollupBySchool({
    scope,
    feeRows: feesOutcome.available ? fees.rows : [],
    attendanceRows: attendanceOutcome.available ? attendance.rows : [],
    academicYear,
    attendanceMonth,
  });

  if (scope.length > 1) {
    const worstRealisation = extremeBy(rollups, (r) => r.realisation, 'min');
    const bestRealisation = extremeBy(rollups, (r) => r.realisation, 'max');
    const mostOwed = extremeBy(rollups, (r) => r.pending, 'max');
    const worstAttendance = extremeBy(rollups, (r) => r.attendance, 'min');
    const rated = rollups.filter((r) => r.realisation !== null);

    const highlights: { highlight: string; school_name: string; value: string }[] = [];
    /**
     * Only when the schools actually DIFFER. Two schools that both realise 99.8%
     * have no "lowest", and printing one anyway invents a laggard out of a
     * rounding difference — the reader then opens a school that has nothing
     * wrong with it. `bestRealisation` is compared rather than a tolerance being
     * invented: if the extremes are the same school, there is one value.
     */
    if (
      worstRealisation !== null &&
      bestRealisation !== null &&
      worstRealisation.row.schoolId !== bestRealisation.row.schoolId
    ) {
      highlights.push(
        {
          highlight: 'Lowest fee realisation',
          school_name: worstRealisation.row.name,
          value: formatPct(worstRealisation.value),
        },
        {
          highlight: 'Best fee realisation',
          school_name: bestRealisation.row.name,
          value: formatPct(bestRealisation.value),
        },
      );
    }
    if (mostOwed !== null && mostOwed.value > 0) {
      highlights.push({
        highlight: 'Largest outstanding',
        school_name: mostOwed.row.name,
        value: formatRupees(mostOwed.value),
      });
    }
    if (worstAttendance !== null) {
      highlights.push({
        highlight: `Lowest attendance${attendanceMonth === null ? '' : ` · ${monthLabel(attendanceMonth)}`}`,
        school_name: worstAttendance.row.name,
        value: formatPct(worstAttendance.value),
      });
    }
    if (rated.length > 1) {
      const mean = rated.reduce((sum, r) => sum + (r.realisation ?? 0), 0) / rated.length;
      highlights.push({
        highlight: 'Average school realisation',
        /**
         ...
         *
         * This is the mean of the SCHOOLS' rates, where the tile above is
         * money-weighted, and the gap between them is the point: a trust whose
         * average school realises 88% while its money realises 93% is a trust
         * whose small schools are the problem.
         */
        school_name: `across ${String(rated.length)} schools`,
        value: formatPct(mean),
      });
    }
...
/**
 * One school's figures for the year on screen, gathered from the rows the KPI
 * strip already read.
 *
 * -- Why this costs no query -------------------------------------------------
 * `run_multi` tags every row with its `school_id` (ADR-011), and the four
 * statements Home runs are already grouped by academic year. So the per-school
 * breakdown is a regrouping of numbers this function has in hand, not a fifth
 * fan-out. That matters more here than anywhere else in the product: Home is
 * the screen every user of every school loads, and Invariant 1 makes each
 * avoidable query against a school database a cost the ERP does not have to pay.
 *
 * A field is `null` where the school has no rows for the year rather than 0.
 * The distinction is the same one the tiles draw: a school that has not rolled
 * its roll over has no student count, and ranking it as the lowest would be
 * reporting an absence as a finding.
 */
...
function rollupBySchool(args: {
  scope: readonly { school_id: string; school_name: string }[];
  feeRows: readonly Record<string, unknown>[];
  attendanceRows: readonly Record<string, unknown>[];
  academicYear: string | null;
  attendanceMonth: string | null;
}): SchoolRollup[] {
  return args.scope.map((school) => {
    const fees = args.feeRows.filter(
      (row) => row['school_id'] === school.school_id && row['ay'] === args.academicYear,
    );
    const attendance = args.attendanceRows.filter(
      (row) => row['school_id'] === school.school_id && row['ym'] === args.attendanceMonth,
    );
    const payable = fees.length === 0 ? null : fees.reduce((n, r) => n + toNumber(r['payable']), 0);
    const paid = fees.length === 0 ? null : fees.reduce((n, r) => n + toNumber(r['paid']), 0);
    const pending = fees.length === 0 ? null : fees.reduce((n, r) => n + toNumber(r['n']), 0);
    const marked = attendance.reduce((n, r) => n + toNumber(r['marked']), 0);
    const present = attendance.reduce((n, r) => n + toNumber(r['present']), 0);
    return {
      ...
      pending,
      /**
       * Collected over demand, from that school's own summed totals. Never an
       * average of anything: averaging rates would weight a 200-pupil school
       * like a 4,000-pupil one, which is the same reason the Fee Collection
       * dashboard computes its realisation from sums (services/dashboards.ts).
       */
      realisation: payable !== null && paid !== null && payable > 0 ? paid / payable : null,
      attendance: marked > 0 ? present / marked : null,
    };
  });
}
...
/** The entry with the smallest/largest value of `pick`, ignoring schools that have none. */
