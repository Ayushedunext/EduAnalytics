...
*Catalog arrangement (2026-09-01):* the catalog above is unchanged — no report was added, removed, renamed or re-scoped — but it is now **grouped into seven modules** for navigation: **Fees · Student · Staff · Attendance · Transport · Exam · General** (`apps/orchestrator/src/services/modules.ts`, UX in docs/10 §2). A module has no definition, no SQL and no serving path; it is a view of this catalog and nothing more, so opening a report through one runs exactly the report §1 defines, through the same `buildDashboard`, under the same ADR-011 fan-out and the same double scope check.

Membership is declared **on the report card** (`DashboardCard.modules`) rather than in a table keyed by report id, so a new report does not compile without a module and the two lists cannot drift into a tile whose names disagree with what opens inside it. **A report may belong to more than one module** where it genuinely reports on more than one thing — Trend Analysis is in Fees and Student because it charts six years of collection and twelve of enrollment, and Staff Attendance is in Staff and Attendance because two different people read it for two different reasons. That is one report reached through two doors, never a duplicated definition. What does *not* get multiple membership is a report that merely touches several domains: the Principal's Snapshot spans students, fees, staff, admissions and attendance, and sits in **General** alone, because padding four modules with the same card tells a reader nothing about which module they are in.

**Exam is the module with nothing in it, and it is served anyway.** Its one report, Exam Performance, is `blocked` for want of any exam data in the ERP extract (AUDIT_REPORT C20), so the tile renders inert carrying that card's own reason. The catalog is the source of that sentence, not the module: when an extract lands, the card turns `available` and the tile fills itself in. docs/10 §3 carries why a tile is allowed to exist where a menu row would be withheld.
