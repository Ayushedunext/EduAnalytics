...
*Admissions Funnel* (`students_admission_data_set`: enquiry → registration → application → admission, plus `candidate_statusid`) is a viable fifth if Phase 1 has room. **Attendance Analytics and Exam Performance move to Phase 3** with the remaining predefined dashboards, conditional on that data existing in the per-school databases — an open question against the ERP team (§2 item 6). The full 15-dashboard catalog in docs/06 §2 is unchanged; only the Phase 1 subset moved.
