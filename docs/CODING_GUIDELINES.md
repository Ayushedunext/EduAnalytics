...
- **[MANDATORY] Identifiers are allowlisted, because they cannot be parameterized.** A placeholder is not legal SQL in the position of a database, table or column name, so `USE ?` and `FROM ?.students` are impossible — and per-school tenancy (docs/03 §2) puts the registry's `db_name` directly into a FROM clause. That is the one string-concatenation site the design cannot remove. Therefore: identifiers are validated against the allowlists in `@sap/shared` (`safeIdSchema`, `safeDbNameSchema`) **where they enter the system** — registry sync, token parsing — so an unsafe identifier cannot exist in a validated object; and they reach SQL only via `quoteMySqlIdentifier()`, never a raw template. MySQL system databases (`mysql`, `information_schema`, `performance_schema`, `sys`) are denied separately: they are syntactically valid but would expose every database on a shared instance. Note that "it came from the ERP sync" is not a reason to skip this — a signed token and a trusted upstream are guarantees of *authenticity*, never of *content*.
