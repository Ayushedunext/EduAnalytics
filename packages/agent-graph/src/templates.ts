 ...
 * §8) — this array is that data. Only `absence-alert`'s `buildGraph` produces
 * a graph apps/agent-runtime can actually run today (its trigger and fetch
 * source are in `RUNNABLE_TRIGGER_KINDS`/`FETCH_SOURCES`); the rest render in
 * the gallery per docs/07 §6 but `buildGraph` returns `null` for them —
 * "not built yet", not hidden, matching this codebase's locked-≠-hidden
 * convention (docs/10 §3) applied to a gallery tile instead of a nav row.
 */
 ...
 */
export const SEED_APPROVED_TEMPLATE_IDS: readonly string[] = ['absence-3rd-day', 'absence-today'];
